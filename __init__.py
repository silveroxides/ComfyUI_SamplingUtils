...
class TextEncodeKleinThinking(io.ComfyNode):
    ...
    def execute(
        ...
        # CRITICAL: Load model to GPU using ComfyUI's model management
        # This is required - the model sits on CPU until this is called
        clip.load_model()
        device = clip.patcher.load_device
        
        # Set execution device on the clip model (like encode_from_tokens does)
        cond_model.set_clip_options({"execution_device": device})
